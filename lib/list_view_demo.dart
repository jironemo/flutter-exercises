  ...
  return ListView.separated(
    itemBuilder: (context, index) {
      return const ListTile(
        ...
        title: Text("Hello"),
        subtitle:
            Text("This is a subtitle", style: TextStyle(color: Colors.grey)),
      );
    },
...
class DynamicListViewWithSeparatorDemo extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return MaterialApp(home: Scaffold(body: dynamicSeparatedList()));
  }
}
...
class DynamicListViewDemo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      itemBuilder: (context, index) {
        return ListTile(
            leading: const Icon(Icons.add), title: Text("Item ${index + 1}"));
      },
    )));
  }
}
